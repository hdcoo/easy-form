...
import { InternalHooks, InternalNamePath, NamePath } from 'rc-field-form/es/interface';
import { HOOK_MARK, FieldContext, valueUtil } from './helper/rcForm';
import { ChangeInfo, ChangeType, FakeField, Trigger } from './core/FieldEntity';
import { lookUpward } from './helper/utils';
import useMounted from './helper/useMounted';

interface UseFieldsChangeOptions {
...
function useFieldsChange<T extends any[] = any[]>(
  ...
  options: UseFieldsChangeOptions = {},
) {
  const {
    mode = 'local',
    immediate = false,
  ...
  const getValues = useCallback((store: any) => {
    if (mode === 'global') {
      return dependenciesCache.current.map((name) => getValue(store, getNamePath(name)));
    }

    return dependenciesCache.current.map((name) => {
      return getValue(store, [...lookUpward(prefixName, upward), ...getNamePath(name)]);
    });
  }, [mode, prefixName, upward]);

  const handleChange = useCallback((e: ChangeInfo) => {
  ...
  useEffect(() => {
    ...
    const unregister = registerField(new FakeField(handleChange, {
      names: dependenciesCache.current.map((dep) => {
        return isGlobal
          ? getNamePath(dep)
          : [...lookUpward(prefixName, upward), ...getNamePath(dep)];
      }),
      trigger,
      requirePathExist,
    }));
