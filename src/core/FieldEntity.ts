import { InternalNamePath, Store, ValuedNotifyInfo, FieldEntity, NamePath } from 'rc-field-form/es/interface';
import { valueUtil } from '../helper/rcForm';
...
abstract class AbstractFakeField implements FieldEntity {
  ...
  protected constructor(options: FieldEntityOptions) {
    ...
    this.props = { name: ['__FAKE_FIELD__'] };
    this.names = names;
  ...
  private checkTrigger = (
    ...
    switch (this.trigger) {
      case 'contain': return this.isContain(name, namePathList);
      case 'exact': return this.isEqual(name, namePathList);
      case 'lower': return this.isLower(name, namePathList);
      default: return true;
    }
  };
  ...
  private isEqual = (path: InternalNamePath, namePathList: InternalNamePath[]) => {
    return !!namePathList && containsNamePath(namePathList, path);
  };
  ...
  private isLower = (path: InternalNamePath, namePathList: InternalNamePath[]) => {
    ...
    for (const namePath of namePathList) {
      if (
        namePath.length >= path.length &&
        path.every((item, i) => item === namePath[i])
      ) {
        return true;
      }
    }
